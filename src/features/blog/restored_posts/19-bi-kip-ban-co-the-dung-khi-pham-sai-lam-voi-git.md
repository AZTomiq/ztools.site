...
> **Cảnh báo:** Những nội dung sau rất nguy hiểm, nếu bạn làm theo, xin hãy chịu trách nhiệm về bản thân mình.
> 
...
Khi làm việc ở nhà và khi làm việc trên công ty thì hẳn sẽ có lúc sử dụng git config khác nhau và cũng có lúc bị nhầm thiết lập.

Ở đây chúng ta đang nói về việc thay đổi tác giả (Author). Hãy đổi Committer ở `.gitconfig` hoặc `.git/config`.
...
> **Lưu ý:** Lịch sử commit sẽ bị thay đổi hoàn toàn
...
> **Lưu ý:** Thay vì dùng squash có thể dùng fixup nhưng khi đó commit message của những commit được fixup sẽ bị mất.
...
# Nếu các phần nhỏ đó vẫn chưa đủ nhỏ thì dùng (s) để tiếp tục chia nhỏ hơn 
# (Chi tiết của git add -p thì hãy hỏi anh Google nhé)
$ git add -p
...
Có những lúc ta muốn 1 phát xoá tất tần tật những file tự động sinh ra hoặc những file tự động backup. Nếu không phải là những file cần git quản lý hoặc những file sẽ commit thì xoá đi thôi.

> **Lưu ý:** Cái này cũng không phải là sự cố nhỉ…
