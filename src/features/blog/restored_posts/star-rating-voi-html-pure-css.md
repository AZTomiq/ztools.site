...
tags: [CSS, Font Awesome, Frontend, HTML, JavaScript, Pure CSS, Star Rating, UI/UX]
...
.rating:not(:checked) > label:hover ~ label { /* Hover các sao trước đó */
    color: #FFD700; 
}
...
function calcRate(rating) {
    const targetStar = rating === 5 ? 'star5'
        : rating >= 4.5 ? 'star4half'
        : rating >= 4 ? 'star4'
        : rating >= 3.5 ? 'star3half'
        : rating >= 3 ? 'star3'
        : rating >= 2.5 ? 'star2half'
        : rating >= 2 ? 'star2'
        : rating >= 1.5 ? 'star1half'
        : rating >= 1 ? 'star1'
        : rating >= 0.5 ? 'starhalf'
        : null;
