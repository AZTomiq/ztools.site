...
This document outlines the strategy for ensuring consistent subdomain usage across the iZTools ecosystem. The goal is to enforce canonical URLs for specific features (e.g., `laikep.iZTools.site` for Compound Interest) and prevent duplicate content or navigation issues.
...
| `compound-interest` | `laikep.iZTools.site`     | `/compound-interest/` |
| `json-toolkit`      | `json.iZTools.site`       | `/json-toolkit/`      |
| `tax`               | `tncn.iZTools.site`       | `/tax/`               |
| `web-playground`    | `playground.iZTools.site` | `/web-playground/`    |
...
- **Rule**: If a user visits `iZTools.site/compound-interest/`, they are 301 Redirected to `https://laikep.iZTools.site/`.
...
- **Config**:
  ```json
  {
    ...
    "has": [{ "type": "host", "value": "iZTools.site" }],
    "destination": "https://laikep.iZTools.site/:path*",
    "permanent": true
  }
  ```
...
Global links (About, Privacy, Terms) are now forced to use `global.site.url` (`https://iZTools.site`) to ensure they always point to the main domain, even when viewed from a subdomain.

## Verification Checklist

1. **Direct Access**:
   - Visit `iZTools.site/tax` -> Should redirect to `tncn.iZTools.site`.
   - Visit `tncn.iZTools.site` -> Should load Tax tool.

2. **Navigation**:
   - On `tncn.iZTools.site`, click "Compound Interest" in Mega Menu.
   - It should navigate to `https://laikep.iZTools.site/`.

3. **Footer Links**:
   - On `tncn.iZTools.site`, click "Privacy Policy".
   - It should navigate to `https://iZTools.site/privacy/`.

4. **Deep Linking**:
   - Visit `iZTools.site/tax/en/` -> Should redirect to `tncn.iZTools.site/en/`.
