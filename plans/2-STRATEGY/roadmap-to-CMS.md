...
Tài liệu này vạch ra lộ trình chuyển đổi hệ thống iZTools từ Custom SSG thuần túy sang mô hình **Git-based / Hybrid CMS**, cho phép quản lý nội dung mà không cần thao tác code thủ công.
...
_Mục tiêu: Chuyển đổi cấu trúc folder dự án sang dạng Atomic/Modular. Mỗi tính năng (tool) sẽ tự quản lý toàn bộ tài nguyên của mình (View, Logic, Style, Content)._
...
_Mục tiêu: Cho phép sửa nội dung JSON/Config ngay trên trình duyệt khi chạy local (`npm run dev`), thay vì sửa file code._
...
_Mục tiêu: Chuẩn hóa dữ liệu để máy có thể hiểu và tự động sinh code/template._
...
- [x] Định nghĩa **Schema** cho một "Tool":
  ...
  actions: ...
  ```
  _(Đã áp dụng mẫu cho Password Generator)_
...
_Mục tiêu: Đưa trang Admin lên môi trường production an toàn._
